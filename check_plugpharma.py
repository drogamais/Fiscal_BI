...
def check_table_status(conn, table_name, asset_type, date_column):
    ...
    Args:
        ...
        date_column (str): O nome da coluna de data a ser usada na query (ex: 'data_insercao').
    ...
    try:
        # A query agora usa a variável 'date_column' para ser flexível
...
def main():
    ...
    # --- DICIONÁRIO DE CONFIGURAÇÃO DAS TABELAS ---
    # Agora, cada tabela tem uma chave 'coluna' para especificar qual
    # campo de data usar na verificação.
    tabelas_para_checar = [
        {'nome': 'bronze_plugpharma_estoque',               'tipo': 'TABELA BRONZE', 'coluna': 'data_insercao'},
        {'nome': 'bronze_plugpharma_vendas',                'tipo': 'TABELA BRONZE', 'coluna': 'data_insercao'},
        #{'nome': 'silver_plugpharma_estoque',               'tipo': 'TABELA SILVER', 'coluna': 'data_insercao'},
        #{'nome': 'silver_plugpharma_vendas',                'tipo': 'TABELA SILVER', 'coluna': 'data_insercao'},
        {'nome': 'silver_plugpharma_lojas_camapanhaPrincipia', 'tipo': 'TABELA SILVER', 'coluna': 'data_de_criacao'},
        {'nome': 'silver_plugpharma_vendas_campanhaPrincipia','tipo': 'TABELA SILVER', 'coluna': 'data_atualização'}
    ]
    ...
    try:
        for tabela in tabelas_para_checar:
            # Passamos o nome da coluna como um novo argumento para a função
